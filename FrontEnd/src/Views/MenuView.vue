  ...
  <div class="menu-container">
    ...
    <div v-else class="page-layout">
      <aside class="cart-sidebar">
        <h2 class="cart-title">Carrinho de <span class="highlight">Compras</span></h2>

        <div v-if="cartStore.items.length === 0" class="empty-cart">Seu carrinho está vazio.</div>
        <div v-else class="cart-items">
          <div v-for="item in cartStore.items" :key="item.productId" class="cart-item">
            <div class="cart-item-info">
              <p class="cart-item-name">{{ item.product }}</p>
              <span class="cart-item-price"
                >R$ {{ ((item.price || 0) * item.quantity).toFixed(2) }}</span
              >
            </div>
            ...
            <div class="cart-item-actions">
              <div class="qty-box">
                <button @click="cartStore.decreaseQuantity(item.productId)" class="btn-qty">
                  -
                </button>
                <span class="qty-val">{{ item.quantity }}</span>
                <button @click="cartStore.increaseQuantity(item.productId)" class="btn-qty">
                  +
                </button>
              </div>
...
const getMenu = async () => {
  try {
    const response = await api.get(`${import.meta.env.VITE_API_URL}/menu`, {
      headers: {
